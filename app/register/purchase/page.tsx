...
export default function StablePurchaseRegistrationPage() {
  ...
  useEffect(() => {
    ...
    if (!identifier) { setError('No product identifier was received.'); setLoading(false); return }
    const storageKey = `olitec-registration-${identifier}`
    const saved = localStorage.getItem(storageKey)
    if (saved) { try { setForm({ ...emptyForm, ...JSON.parse(saved) }) } catch { localStorage.removeItem(storageKey) } }
    getProduct(identifier).then(item => { if (!cancelled) setProduct(item) }).catch(e => { if (!cancelled) setError(e instanceof Error ? e.message : 'Product could not be verified.') }).finally(() => { if (!cancelled) setLoading(false) })
    return () => { cancelled = true }
  }, [])

  const update = (key: keyof RegistrationForm, value: string) => setForm(prev => ({ ...prev, [key]: value }))
  const storageKey = serial ? `olitec-registration-${serial}` : ''
  ...
  function submit(e: FormEvent) {
    ...
    router.push(`/register/review?identifier=${encodeURIComponent(serial)}`)
  }

  if (loading) return <div className="app"><main><section className="card"><p>Loading registration…</p></section></main></div>
  if (error || !product) return <div className="app"><main><section className="card"><h2>Unable to continue</h2><p>{error}</p><button className="btn secondary" onClick={() => router.push(`/register?identifier=${encodeURIComponent(serial)}`)}>Go Back</button></section></main></div>

  return <div className="app">
    <header><div style={{fontWeight:800,fontSize:28,letterSpacing:1}}>OLITEC</div><div className="lang">English⌄</div></header>
    <main>
      <button className="back" onClick={() => router.push(`/register?identifier=${encodeURIComponent(serial)}`)}>← Product Verification</button>
