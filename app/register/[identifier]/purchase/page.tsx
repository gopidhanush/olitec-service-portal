...
export default function PurchaseRegistrationPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (saved) { try { setForm(JSON.parse(saved) as RegistrationForm) } catch { localStorage.removeItem(storageKey) } }
  ...
  async function submit(e: FormEvent) {
    ...
    setError(''); setSaving(true); localStorage.setItem(storageKey, JSON.stringify(form)); setSaving(false)
      ...
      <main className="customerMain">
        ...
        <form onSubmit={submit}>
          ...
          <section className="customerSection">
            ...
            <label>Invoice upload</label><div className="customerUpload">📄<strong>Upload invoice</strong><small>PDF, JPG or PNG</small></div>
          </section>
          ...
          <button className="customerButton customerButtonPrimary customerSubmit" disabled={saving}>{saving ? 'Saving…' : 'Continue to Review'} <span>→</span></button>
        </form>
      </main>
      <PortalFooter />
    </div>
