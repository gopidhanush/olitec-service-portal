...
export default function RegisterLookupPage() {
  ...
  useEffect(() => {
    ...
    async function loadProduct() {
      ...
      if (!identifier) {
        if (!cancelled) {
          setError('No product identifier was received from the QR code.')
          setLoading(false)
        }
        return
      }
      ...
      if (!supabaseUrl || !publishableKey) {
        if (!cancelled) {
          setError('Product verification is not configured. Please contact OLITEC support.')
          setLoading(false)
        }
        return
      }
      ...
      try {
        ...
        try {
          ...
          throw new Error('Invalid response from product verification service')
        }

        if (!response.ok) {
          console.error('Product verification RPC error:', response.status, body)
          throw new Error('Product verification service returned an error')
        }

        if (!data.length) {
          throw new Error('Product not found. Please check the QR code and try again.')
        }

        if (!cancelled) setProduct(data[0])
      } catch (e) {
        console.error(e)
        if (!cancelled) setError(e instanceof Error ? e.message : 'Unable to verify this product.')
      } finally {
      ...
      <main>
        ...
        <div className="hero">
          <div className="heroText">
            ...
            <p>We are checking the OLITEC product linked to this QR code.</p>
          </div>
        </div>
        ...
        {!loading && error && (
          <section className="card">
            <h2>Product Not Verified</h2>
            <p>{error}</p>
            <button className="btn secondary" onClick={() => router.push('/')}>Scan Again</button>
          </section>
        )}
