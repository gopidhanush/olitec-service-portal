...
function ProductImage({item}:{item:Warranty}){
  const src=imageUrl(item.product_image)
  ...
  useEffect(()=>{
    ...
    preload.onerror=()=>setFailed(true)
    preload.src=src
    return()=>{preload.onload=null;preload.onerror=null}
  },[src])
  const displaySrc=failed?'/olitec-generated-hero.jpg':src
  return <div style={{width:180,height:180,border:'1px solid #dfe6ee',borderRadius:22,backgroundColor:'#f8fafc',backgroundImage:`url("${displaySrc}")`,backgroundPosition:'center',backgroundRepeat:'no-repeat',backgroundSize:'contain',display:'grid',placeItems:'center',overflow:'hidden'}}>
    <img key={displaySrc} src={displaySrc} alt={`${item.model_code} solar inverter`} loading="eager" decoding="async" referrerPolicy="no-referrer" style={{width:'100%',height:'100%',objectFit:'contain',display:'block',opacity:loaded||failed?1:0,transition:'opacity .15s ease'}} onLoad={()=>setLoaded(true)} onError={()=>setFailed(true)} />
  </div>
}
...
export default function WarrantyVerificationPage(){
 ...
 {selected&&<div style={{display:'flex',justifyContent:'center',marginBottom:22}}><ProductImage item={selected}/></div>}
