...
async function getWarranty(registration: string): Promise<Warranty | null> {
  ...
  if (!response.ok) {
    ...
    console.error('Warranty verification RPC failed:', response.status, detail)
    throw new Error(`Warranty verification failed (${response.status})`)
  }
  ...
  useEffect(() => {
    ...
    getWarranty(registration)
      ...
      .catch((err) => {
        ...
        if (active) setError('We could not verify this warranty registration right now. Please try again.')
      })
      .finally(() => {
