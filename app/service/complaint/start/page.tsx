...
export default function ComplaintStartPage() {
  ...
  function submit(event: FormEvent) { event.preventDefault(); const value = identifier.trim().toUpperCase(); if (value) window.location.href = `/service/complaint?identifier=${encodeURIComponent(value)}` }
  return <div className="app customerPage portalComplaintPage cleanPortalPage"><main className="customerMain cleanMain"><div className="customerSplitLayout cleanSplitLayout">
    ...
    <section className="customerSection customerFormPanel cleanFormPanel"><span className="customerBadge">Service Complaint</span><h2>Find your product</h2><p>Enter either the registration number or the serial number of your registered OLITEC product.</p><form onSubmit={submit}><label>Registration number or serial number</label><input className="customerInput" required autoCapitalize="characters" autoComplete="off" value={identifier} onChange={e => setIdentifier(e.target.value.toUpperCase())} placeholder="Enter registration or serial number" /><button className="customerButton customerButtonPrimary" type="submit">Continue <span>→</span></button></form></section>
  </div></main></div>
}
