  ...
  useEffect(() => {
      ...
      try {
        ...
        const scan = async () => {
          ...
          if (!detecting && video.readyState >= 2 && video.videoWidth > 0) {
            ...
            const height = Math.min(video.videoHeight, Math.round(width * video.videoHeight / video.videoWidth))
          ...
          if (mounted && !handled) timerRef.current = window.setTimeout(() => { void scan() }, 90)
        }
        timerRef.current = window.setTimeout(() => { void scan() }, 80)
        return true
      } catch (nativeError) {
    ...
    async function startZXing() {
      try {
        ...
        const controls = await reader.decodeFromConstraints(cameraConstraints, videoRef.current, result => {
