...
-- Private invoice storage. The frontend uploads invoices through the storage API;
-- invoice paths are stored on the registration record, not exposed through a table policy.
...
-- Product verification RPC used by the QR/serial registration page.
-- The image is read defensively from common image fields using JSON so this function
-- remains compatible with the existing products/product_models schema.
...
as $$
  ...
  where upper(trim(p.serial_number)) = upper(trim(identifier))
     or upper(trim(p.qr_code)) = upper(trim(identifier))
  limit 1;
$$;
